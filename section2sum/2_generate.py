...
def generate_summaries_examples(
    ...
    **generate_kwargs,
    ...
    generated_sents = model.tokenizer.batch_decode(summaries, skip_special_tokens=True, clean_up_tokenization_spaces=False)
    return generated_sents

def run_generate():
    #####################################
    # input from data, not used
    #####################################
    # data = joblib.load(join('/home/amax/zzhaoao/BookComprehension/FinetuneBART/data', 'data.pkl'))['test']
    # src_lines = []
    # tgt_lines = []
    # q_lines = []
    # prefix = []
    ...
    #         src_lines.append('{} {} '.format(TYPE2CONTROL_SIGNAL[question_type],
    #                                          NUMBER_CONTROL_SIGNAL[type_counter[question_type]]) + section)
    #         prefix.append('{} {} '.format(TYPE2CONTROL_SIGNAL[question_type], 
    #                                       NUMBER_CONTROL_SIGNAL[type_counter[question_type]]))
    #         tgt_lines.append(inference)
    #         q_lines.append(question)
            
    # with open(join(args.input_path, 'test.source'), 'w') as source, open(join(args.input_path, 'test.target'), 'w') as target:
    #     for (sline, tline) in zip(src_lines, tgt_lines):
    #         source.write(sline + '\n')
    #         target.write(tline + '\n')
    ...
    result_file= os.path.join(args.output_dir, 'pred_sum.txt')
